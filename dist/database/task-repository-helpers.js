 ...
 */
export async function validateTaskInput(status, statusRepository) {
    if (status) {
 ...
 */
export async function prepareTaskMetadata(task, statusRepository) {
    const statuses = await statusRepository.getAllStatuses();
 ...
 */
export async function registerTaskTags(tags, tagRepository) {
 ...
 */
export function buildTaskSyncParams(task, type, statusId) {
    return [
        type,
        task.id,
        task.title,
 ...
 */
export async function validateUpdateInput(status, statusRepository) {
    if (status !== undefined) {
        const statuses = await statusRepository.getAllStatuses();
        const isValid = statuses.some(s => s.name === status);
        if (!isValid) {
 ...
 */
export function mergeTaskData(existingTask, updates) {
...
//# sourceMappingURL=task-repository-helpers.js.map