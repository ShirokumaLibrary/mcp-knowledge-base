...
export class OptimizedBaseRepository extends BaseRepository {
    queryCache;
    entityCache;
    constructor(db, tableName, loggerName, cacheConfig) {
        super(db, tableName, loggerName);
        // Initialize caches
        this.queryCache = new MemoryCache(cacheConfig?.maxSize || 50, cacheConfig?.ttl || 30000 // 30 seconds default
        );
        this.entityCache = new MemoryCache(cacheConfig?.maxSize || 100, cacheConfig?.ttl || 60000 // 1 minute default
        );
    }
    /**
    ...
    async findByIds(ids) {
        ...
        if (missingIds.length > 0) {
            ...
            const rows = await this.executeQuery(query, missingIds);
...
//# sourceMappingURL=optimized-base-repository.js.map