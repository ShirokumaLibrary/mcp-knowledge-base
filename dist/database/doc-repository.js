...
export class DocRepository extends BaseRepository {
    docsDir;
    tagRepository;
    constructor(db, docsDir, tagRepository) {
    ...
    async getAllDocs() {
        ...
        const docs = results.filter((doc) => doc !== null);
    ...
    async searchDocsByTag(tag) {
        ...
        const docRows = await this.db.allAsync(`SELECT DISTINCT d.id 
       FROM search_docs d
       JOIN doc_tags dt ON d.id = dt.doc_id
       WHERE dt.tag_id = ?
       ORDER BY d.id`, [tagRow.id]);
        // Load full doc data from search table for better performance
        const docs = [];
        for (const row of docRows) {
            const docData = await this.db.getAsync('SELECT * FROM search_docs WHERE id = ?', [row.id]);
            if (docData) {
                docs.push({
                    id: docData.id,
                    title: docData.title,
