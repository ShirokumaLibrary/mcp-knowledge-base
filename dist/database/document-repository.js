...
export class DocumentRepository extends BaseRepository {
    ...
    async getAllDocumentPatterns() {
        ...
        try {
            ...
            for (const subdir of subdirs) {
                if (subdir === 'docs' || subdir === 'knowledge')
                    continue;
    ...
    async deleteDocument(type, id) {
        ...
        try {
            ...
            await this.db.runAsync('DELETE FROM search_documents WHERE type = ? AND id = ?', [type, id]);
            return true;
        }
        catch (error) {
            return false;
    ...
    async getAllDocumentsSummary(type) {
        ...
        const rows = await this.db.allAsync(`SELECT type, id, title, summary as description, tags, created_at, updated_at 
    ...
    async searchDocumentsByTag(tag, type) {
        ...
        const rows = await this.db.allAsync(`SELECT DISTINCT d.type, d.id 
     ...
     */
    async syncDocumentToSQLite(document) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        await this.db.runAsync('DELETE FROM document_tags WHERE document_type = ? AND document_id = ?', [document.type, document.id]);
        if (tagList.length > 0) {
    ...
    `);
        // Create indexes
        await this.db.runAsync(`CREATE INDEX IF NOT EXISTS idx_documents_text 
       ON search_documents(title, content, summary)`);
        await this.db.runAsync(`CREATE INDEX IF NOT EXISTS idx_documents_tags 
       ON search_documents(tags)`);
        await this.db.runAsync(`CREATE INDEX IF NOT EXISTS idx_document_tags_lookup 
