...
Multiple lines
of content.`;
            await fs.writeFile(path.join(tempDir, 'issues', 'issues-1.md'), issueContent, 'utf-8');
...
Plan content`
        ...
        it('should handle malformed files gracefully', async () => {
            ...
            await fs.writeFile(path.join(tempDir, 'docs', 'docs-1.md'), `---
title: Valid Doc
---
Content`, 'utf-8');
            // Invalid: no frontmatter
            await fs.writeFile(path.join(tempDir, 'docs', 'docs-2.md'), 'Just content without frontmatter', 'utf-8');
            // Invalid: wrong filename pattern
            await fs.writeFile(path.join(tempDir, 'docs', 'invalid.md'), `---
...
Content`
        ...
        it('should update sequences correctly', async () => {
            ...
            await fs.writeFile(path.join(tempDir, 'issues', 'issues-5.md'), `---
title: Issue 5
---
Content`, 'utf-8');
            await fs.writeFile(path.join(tempDir, 'issues', 'issues-10.md'), `---
title: Issue 10
---
Content`, 'utf-8');
            const syncedCount = await itemRepo.rebuildFromMarkdown('issues');
...
- Здравствуйте
- مرحبا`;
            await fs.writeFile(path.join(tempDir, 'knowledge', 'knowledge-1.md'), content, 'utf-8');
            const syncedCount = await itemRepo.rebuildFromMarkdown('knowledge');
            expect(syncedCount).toBe(1);
            const items = await itemRepo.getItems('knowledge');
...
//# sourceMappingURL=item-repository-rebuild.test.js.map