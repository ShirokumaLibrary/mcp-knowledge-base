...
describe('DocRepository Async Tests', () => {
    ...
    describe('Async file operations', () => {
        ...
        test('should create doc with async file write', async () => {
            const doc = await db.createDoc('Test Document', '# Test Content\n\nThis is a test document.', // @ai-example: Markdown content
            ['test', 'async']);
        ...
        test('should handle concurrent doc operations', async () => {
            ...
            const promises = Array.from({ length: 5 }, (_, i) => db.createDoc(`Concurrent Doc ${i}`, `Content for document ${i}`, [`tag${i}`]));
        ...
        test('should update doc with async file operations', async () => {
            ...
            const updateResult = await db.updateDoc(doc.id, 'Updated Title', 'Updated content with more details', ['updated', 'test']);
        ...
        test('should handle getDocsSummary with async operations', async () => {
            ...
            expect(Object.keys(shortDocSummary).sort()).toEqual(['description', 'id', 'title']);
            expect(Object.keys(longDocSummary).sort()).toEqual(['description', 'id', 'title']);
        });
        /**
...
//# sourceMappingURL=doc-repository.test.js.map