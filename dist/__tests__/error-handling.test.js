...
describe('Error Handling Integration Tests', () => {
    ...
    describe('Database errors', () => {
        it('should handle database lock errors', async () => {
            ...
            const mockDb = jest.spyOn(context.db, 'createTask');
            mockDb.mockRejectedValueOnce(new Error('SQLITE_BUSY: database is locked'));
            // Try to create a task - should get the error
            await expect(context.db.createTask('issues', 'Test Task', 'Content')).rejects.toThrow('SQLITE_BUSY: database is locked');
            mockDb.mockRestore();
        });
        /**
        ...
        it('should handle corrupted database', async () => {
            ...
            const mockAllAsync = jest.spyOn(context.db.getDatabase(), 'allAsync');
