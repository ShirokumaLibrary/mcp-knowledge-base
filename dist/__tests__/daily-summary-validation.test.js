...
describe('Daily Summary Validation', () => {
    ...
    describe('Duplicate Summary Prevention', () => {
        ...
        it('should prevent creating duplicate summaries for the same date', async () => {
            ...
            const summary1 = await sessionManager.createDaily(testDate, 'First Summary', 'This is the first summary', ['test']);
            expect(summary1).toBeDefined();
            expect(summary1.date).toBe(testDate);
            // Try to create duplicate - should throw
            await expect(sessionManager.createDaily(testDate, 'Second Summary', 'This should fail', ['test'])).rejects.toThrow('Daily summary for 2025-07-30 already exists');
        });
        /**
        ...
        it('should allow creating summaries for different dates', async () => {
            const summary1 = await sessionManager.createDaily('2025-07-30', 'July Summary', 'Summary for July', []);
            const summary2 = await sessionManager.createDaily('2025-07-31', 'August Summary', 'Summary for August', []);
...
//# sourceMappingURL=daily-summary-validation.test.js.map