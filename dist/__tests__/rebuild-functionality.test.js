    ...
    describe('Session and Daily Summary Sync', () => {
        test('should sync sessions during rebuild', async () => {
            ...
            const sessionConfig = {
                ...
                filePrefix: 'sessions-',
                useDateSubdir: true,
                dateExtractor: (id) => id.split('-').slice(0, 3).join('-')
            };
            ...
            const sessions = await database.allAsync('SELECT * FROM items WHERE type = ? ORDER BY id', ['sessions']);
        ...
        test('should sync dailies during rebuild', async () => {
            ...
            const database = db.getDatabase();
            const dailies = await database.allAsync('SELECT * FROM items WHERE type = ? ORDER BY id', ['dailies']);
        ...
        test('should handle mixed types during rebuild', async () => {
            ...
            const database = db.getDatabase();
            const allItems = await database.allAsync('SELECT type, COUNT(*) as count FROM items GROUP BY type ORDER BY type');
            const counts = allItems.reduce((acc, row) => {
            ...
            const rebuildItems = await database.allAsync(`SELECT DISTINCT type FROM items WHERE tags LIKE '%"rebuild"%' ORDER BY type`);
            expect(rebuildItems.map((r) => r.type)).toEqual([
        ...
        test('should preserve timestamps during rebuild', async () => {
            ...
            const database = db.getDatabase();
            await database.runAsync('UPDATE items SET created_at = ? WHERE type = ? AND id = ?', [createdAt, 'sessions', session.id]);
            ...
            const updatedContent = sessionContent.replace(/created_at: ".*"/, `created_at: "${createdAt}"`);
            ...
            const rebuiltSession = await newDatabase.getAsync('SELECT * FROM items WHERE type = ? AND id = ?', ['sessions', session.id]);
...
The login button is not working`;
            ...
            const database = db.getDatabase();
            const sequences = await database.allAsync('SELECT type, base_type FROM sequences WHERE type IN (?, ?) ORDER BY type', ['bugs', 'recipe']);
    ...
    describe('Rebuild with ItemRepository', () => {
        test('should rebuild all types using ItemRepository', async () => {
            ...
            try {
                ...
                const beforeCount = await database.getAsync('SELECT COUNT(*) as count FROM items');
                ...
                const afterCount = await database.getAsync('SELECT COUNT(*) as count FROM items');
...
//# sourceMappingURL=rebuild-functionality.test.js.map