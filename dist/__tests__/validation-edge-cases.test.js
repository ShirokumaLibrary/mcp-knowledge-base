...
describe('Validation Edge Cases', () => {
    ...
    describe('Date Validation', () => {
        it('should reject invalid date format for start_date', async () => {
            await expect(db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, '2025-02-30') // Invalid date
            ).rejects.toThrow('Invalid date: 2025-02-30');
        });
        it('should reject invalid date format for end_date', async () => {
            await expect(db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, undefined, '2025-13-01') // Invalid month
            ).rejects.toThrow('Invalid date: 2025-13-01');
        });
        it('should reject February 31st', async () => {
            await expect(db.createTask('plans', 'Test Plan', 'Test content', 'medium', undefined, [], undefined, '2025-02-31')).rejects.toThrow('Invalid date: 2025-02-31');
        });
        it('should reject non-existent leap year date', async () => {
            await expect(db.createTask('plans', 'Test Plan', 'Test content', 'medium', undefined, [], undefined, '2025-02-29') // 2025 is not a leap year
        ...
        it('should reject invalid date when updating', async () => {
            ...
            await expect(db.updateTask('issues', parseInt(created.id), undefined, // title
            undefined, // content
            undefined, // priority
            undefined, // status
            undefined, // tags
        ...
        it('should reject date with invalid day for month', async () => {
            await expect(db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, '2025-06-31') // June has 30 days
    ...
    describe('Duplicate Related Items', () => {
        it('should handle duplicate items in related_tasks gracefully', async () => {
            const created = await db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, undefined, undefined, ['issues-1', 'issues-1', 'issues-2'] // Duplicate issues-1
            );
        ...
        it('should handle duplicate items when updating', async () => {
            const created = await db.createTask('issues', 'Test Issue', 'Test content');
            const updated = await db.updateTask('issues', parseInt(created.id), undefined, // title
            undefined, // content
            undefined, // priority
            undefined, // status
            undefined, // tags
        ...
        it('should silently remove duplicates when updating', async () => {
            const created = await db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, undefined, undefined, ['issues-10', 'issues-20']);
    ...
    describe('Type Name Validation', () => {
        it('should reject type names longer than 50 characters', async () => {
            await expect(db.getTypeRepository().createType('a'.repeat(51))).rejects.toThrow('Type name must be 50 characters or less');
        });
        ...
        it('should reject empty type names', async () => {
            await expect(db.getTypeRepository().createType('')).rejects.toThrow(/at least 1 character/);
        });
    });
    describe('Tag Validation', () => {
        it('should reject tags with only whitespace', async () => {
            await expect(db.createTag('   ')).rejects.toThrow('Tag name cannot be empty or whitespace only');
        });
        ...
        it('should handle whitespace in item tags', async () => {
            const created = await db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, ['  tag1  ', '   ', 'tag2', ''] // Mix of valid and invalid tags
            );
    ...
    describe('Title Length Validation', () => {
        it('should reject titles longer than 500 characters', async () => {
            ...
            await expect(db.createTask('issues', longTitle, 'Content')).rejects.toThrow('Title must be 500 characters or less');
        });
        ...
        it('should reject long titles when updating', async () => {
            ...
            await expect(db.getItemRepository().updateItem({
                type: 'issues',
                id: created.id,
                title: longTitle
    ...
    describe('Related Fields Validation', () => {
        it('should reject empty strings in related_tasks array', async () => {
            await expect(db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, undefined, undefined, ['issues-1', '', 'plans-1'])).rejects.toThrow('Related items cannot contain empty strings');
        });
        it('should reject empty strings in related_documents array', async () => {
            await expect(db.createTask('issues', 'Test Issue', 'Test content', 'medium', undefined, [], undefined, undefined, undefined, undefined, ['docs-1', '', 'knowledge-1'])).rejects.toThrow('Related items cannot contain empty strings');
        });
...
//# sourceMappingURL=validation-edge-cases.test.js.map