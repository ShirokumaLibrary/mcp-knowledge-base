...
export class StatusHandlersV2 extends BaseHandler {
   ...
   */
  handleGetStatuses = this.wrapHandler(
    'get statuses',
    EmptySchema,
    async () => {
      this.ensureDatabase();
   ...
   */
  handleCreateStatus = this.wrapHandler(
    'create status',
    CreateStatusSchema,
    async (args) => {
      this.ensureDatabase();
   ...
   */
  handleUpdateStatus = this.wrapHandler(
    'update status',
    UpdateStatusSchema,
    async (args) => {
      this.ensureDatabase();
   ...
   */
  handleDeleteStatus = this.wrapHandler(
    'delete status',
    DeleteStatusSchema,
    async (args) => {
      this.ensureDatabase();
      ...
      if (!deleted) {
        return this.createErrorResponse(
          `Status with ID ${args.id} not found or is in use by existing items`
        );
      }
