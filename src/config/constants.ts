...
export const ERROR_MESSAGES = {
  ...
  // Entity errors
  ENTITY: {
    NOT_FOUND: (type: string, id: string | number) => `${type} with ID ${id} not found`,
    ALREADY_EXISTS: (type: string, identifier: string) => `${type} ${identifier} already exists`,
    IN_USE: (type: string, id: string | number) => `${type} ${id} is in use and cannot be deleted`,
    CREATE_FAILED: (type: string) => `Failed to create ${type}`,
    UPDATE_FAILED: (type: string, id: string | number) => `Failed to update ${type} ${id}`,
    DELETE_FAILED: (type: string, id: string | number) => `Failed to delete ${type} ${id}`
  },
...
export const FORMAT_CONSTANTS = {
  ...
  // List display
  LIST_DISPLAY: {
    EMPTY_MESSAGE: (type: string) => `No ${type} found`,
    SINGLE_ITEM: (type: string) => `1 ${type} found`,
    MULTIPLE_ITEMS: (type: string, count: number) => `${count} ${type}s found`,
    TRUNCATE_LENGTH: 100,
