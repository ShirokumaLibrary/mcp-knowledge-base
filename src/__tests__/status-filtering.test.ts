...
describe('Status Filtering Tests', () => {
  ...
  describe('Status transitions', () => {
    it('should handle status updates correctly', async () => {
      ...
      expect(tasks.find((t: any) => t.id === String(task.id))).toBeTruthy();
      ...
      expect(tasks.find((t: any) => t.id === String(task.id))).toBeFalsy();
    ...
    it('should validate status names on update', async () => {
      ...
      await expect(
        context.db.updateTask('issues', parseInt(task.id), undefined, undefined, undefined, 'InvalidStatus')
  ...
  describe('Edge cases', () => {
    it('should handle tasks with null status gracefully', async () => {
      ...
      await conn.runAsync(
        'INSERT INTO items (type, id, title, priority, status_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['issues', 999, 'Null Status Task', 'medium', null, now, now]
      );
      
      // Should handle gracefully
      const tasks = await context.db.getAllTasksSummary('issues');
      const nullStatusTask = tasks.find((t: any) => t.id === '999');
    ...
    it('should handle concurrent status updates', async () => {
      ...
      const updates = Array(5).fill(null).map((_, i) => 
        context.db.updateTask('issues', parseInt(task.id), undefined, undefined, undefined, 
