...
describe('TaskRepository', () => {
  ...
  describe('Task operations', () => {
    it('should create an issue task', async () => {
      const issue = await taskRepo.createTask(
        'issues',
        'Test Issue',
        'Issue content',
        'high',
    ...
    it('should handle related_tasks', async () => {
      ...
      const issue2 = await taskRepo.createTask(
        'issues',
        'Issue with related tasks',
        'Content',
        'high',
        undefined,
        ['important'],
      ...
      const updated = await taskRepo.updateTask(
        ...
        undefined,
        undefined,
        ['issues-1', 'plans-1', 'issues-2'] // Note: issues-2 doesn't exist but should still be stored
      );
      
      expect(updated?.related_tasks).toEqual(['issues-1', 'plans-1', 'issues-2']);
      
      // Clear related tasks
      const cleared = await taskRepo.updateTask(
        'issues',
        issue2.id,
        undefined,
        undefined,
    ...
    it('should create a plan task', async () => {
      const plan = await taskRepo.createTask(
        'plans',
        'Test Plan',
        'Plan content',
        'high',
  ...
  describe('SQLite sync', () => {
    ...
    it('should handle tag relationships in task_tags table', async () => {
      ...
      // Check task_tags
      const taskTagRows = await db.allAsync(
        'SELECT * FROM task_tags WHERE task_type = ? AND task_id = ?',
        ['issues', issue.id]
      );
      
      expect(taskTagRows).toHaveLength(2);
      expect(taskTagRows.map((r: any) => r.tag_id).sort()).toEqual(tagIds.sort());
    });
  });
  ...
  describe('Related tasks operations', () => {
    it('should create task with related_tasks', async () => {
      ...
      const task3 = await taskRepo.createTask(
        'issues',
        ...
        undefined,
        undefined,
        [`issues-${task1.id}`, `plans-${task2.id}`]
      );
    ...
    it('should update related_tasks', async () => {
      ...
      const updated = await taskRepo.updateTask(
        ...
        undefined,
        undefined,
        [`issues-${task3.id}`, `plans-${task2.id}`]
      );
