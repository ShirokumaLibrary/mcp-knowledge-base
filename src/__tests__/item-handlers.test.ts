...
describe('ItemHandlers', () => {
  ...
  describe('handleCreateItem', () => {
    ...
    it('should validate status names for tasks', async () => {
      mockDbConnection.getAsync
        .mockResolvedValueOnce({ type: 'issues' })
        .mockResolvedValueOnce({ base_type: 'tasks' });
      mockDb.createTask = jest.fn().mockRejectedValue(
        new Error('Invalid status: InvalidStatus')
      );
  ...
  describe('handleSearchItemsByTag', () => {
    it('should search across all types when types not specified', async () => {
      ...
      const mockPlanResults: any[] = [];
      const mockDocResults = [{ id: 1, title: 'API Doc' }];
      const mockKnowledgeResults: any[] = [];
      ...
      mockDb.searchTasksByTag = jest.fn()
        .mockImplementation((type, tag) => {
          if (type === 'issues' && tag === 'api') return Promise.resolve(mockIssueResults);
          if (type === 'plans' && tag === 'api') return Promise.resolve(mockPlanResults);
          return Promise.resolve([]);
        });
      
      mockDb.searchDocumentsByTag = jest.fn()
        .mockImplementation((tag, type) => {
          if (type === 'docs' && tag === 'api') return Promise.resolve(mockDocResults);
          if (type === 'knowledge' && tag === 'api') return Promise.resolve(mockKnowledgeResults);
          return Promise.resolve([]);
        });
  ...
  describe('Edge cases and error handling', () => {
    ...
    it('should handle concurrent operations', async () => {
      ...
      mockDb.createTask = jest.fn().mockImplementation((type: any, title: any) => 
        Promise.resolve({ 
