...
export class InputSanitizer {
  ...
  static sanitizeString(
    ...
    if (typeof input !== 'string') {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Invalid type', value: input }]
      );
    }
    ...
    // Length check
    const limit = maxLength || MAX_LENGTHS[fieldName as keyof typeof MAX_LENGTHS] || 1000;
    if (sanitized.length > limit) {
      throw new ValidationError(
        `${fieldName} exceeds maximum length of ${limit}`,
        [{ field: fieldName, message: 'Too long', value: sanitized.length }]
      );
    }
    ...
    if (sanitized.length === 0) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Required', value: sanitized }]
      );
    }
    ...
    for (const pattern of SQL_INJECTION_PATTERNS) {
      if (pattern.test(sanitized)) {
        ...
        throw new ValidationError(
          ...
          [{ field: fieldName, message: 'Invalid characters', value: 'hidden' }]
        );
      }
    }
  ...
  static sanitizeNumber(
    ...
    if (isNaN(num)) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Invalid number', value: input }]
      );
    }
    ...
    if (!options?.allowFloat && !Number.isInteger(num)) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Must be integer', value: num }]
      );
    }
    ...
    if (options?.min !== undefined && num < options.min) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Too small', value: num }]
      );
    }
    ...
    if (options?.max !== undefined && num > options.max) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Too large', value: num }]
      );
    }
  ...
  static sanitizeBoolean(input: unknown, fieldName: string): boolean {
    ...
    throw new ValidationError(
      ...
      [{ field: fieldName, message: 'Invalid boolean', value: input }]
    );
  }
  ...
  static sanitizeDate(input: unknown, fieldName: string): string {
    ...
    if (isNaN(date.getTime())) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Invalid date', value: input }]
      );
    }
    ...
    for (const pattern of PATH_TRAVERSAL_PATTERNS) {
      if (pattern.test(path)) {
        ...
        throw new ValidationError(
          ...
          [{ field: fieldName, message: 'Invalid path', value: 'hidden' }]
        );
      }
    }
  ...
  static sanitizeArray<T>(
    input: unknown,
    fieldName: string,
    elementSanitizer: (element: unknown, index: number) => T,
    ...
    if (options?.minLength !== undefined && input.length < options.minLength) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Too few items', value: input.length }]
      );
    }
    ...
    if (options?.maxLength !== undefined && input.length > options.maxLength) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Too many items', value: input.length }]
      );
    }
   ...
   */
  static sanitizeEnum<T extends string>(
    input: unknown,
    fieldName: string,
    ...
    if (!allowedValues.includes(value as T)) {
      throw new ValidationError(
        ...
        [{ field: fieldName, message: 'Invalid value', value }]
      );
    }
      ...
      if (field in inputData) {
        try {
          ...
          throw new ValidationError(
            ...
            [{ field, message: 'Validation error', value: inputData[field] }]
          );
        }
      }
    }
