...
describe('InputSanitizer', () => {
  ...
  describe('sanitizeArray', () => {
    it('should sanitize array elements', () => {
      const numbers = [1, '2', 3];
      const result = InputSanitizer.sanitizeArray(numbers, 'numbers', (item) => 
        InputSanitizer.sanitizeNumber(item, 'number')
      );
    ...
    it('should propagate element validation errors', () => {
      ...
      expect(() => InputSanitizer.sanitizeArray(mixed, 'test', (item) => {
        if (!item) throw new ValidationError('Invalid item', []);
        return item;
  ...
  describe('createSanitizationMiddleware', () => {
    it('should create middleware that sanitizes data', () => {
      const schema = {
        name: (val: unknown) => InputSanitizer.sanitizeString(val, 'name'),
        age: (val: unknown) => InputSanitizer.sanitizeNumber(val, 'age', { min: 0, max: 150 }),
        active: (val: unknown) => InputSanitizer.sanitizeBoolean(val, 'active')
      };
    ...
    it('should handle validation errors', () => {
      const schema = {
        path: (val: unknown) => InputSanitizer.sanitizePath(val, 'path'),
        name: (val: unknown) => InputSanitizer.sanitizeString(val, 'name')
      };
    ...
    it('should skip missing fields', () => {
      const schema = {
        required: (val: unknown) => InputSanitizer.sanitizeString(val, 'required'),
        optional: (val: unknown) => InputSanitizer.sanitizeString(val, 'optional')
      };
