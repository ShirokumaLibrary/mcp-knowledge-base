...
const activeDatabases = new Map<string, DatabaseTracker>();
let dbCounter = 0;
 ...
 */
export async function createTestDatabase(prefix: string): Promise<TestDatabaseContext> {
...
export async function closeAllTestDatabases(): Promise<void> {
  if (activeDatabases.size === 0) return;
