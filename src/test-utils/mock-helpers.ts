 ...
 */
export const mockResolvedValue = (value: any) => {
 ...
 */
export const mockRejectedValue = (error: any) => {
  const fn = jest.fn() as any;
 ...
 */
export const mockReturnValue = (value: any) => {
  const fn = jest.fn() as any;
 ...
 */
export const mockImplementation = (impl: (...args: any[]) => any) => {
  const fn = jest.fn() as any;
...
export function createSimpleMockDatabase() {
  return {
    ...
    getDatabase: mockReturnValue({
      getAsync: mockImplementation(async (query: string, params: any[]) => {
        ...
        if (query.includes('SELECT base_type FROM sequences WHERE type = ?')) {
          ...
          if (taskTypes.includes(params[0])) return { base_type: 'tasks' };
          if (documentTypes.includes(params[0])) return { base_type: 'documents' };
          return null;
        }
        return null;
      }),
 ...
 */
export function createMockRepository<T>(overrides: Partial<T> = {}) {
  const baseMock = {
    getAll: mockResolvedValue([]),
    getById: mockResolvedValue(null),
  ...
  mocks.forEach(mock => {
    if (mock && typeof mock === 'object') {
      Object.values(mock).forEach(value => {
        if (value && typeof value === 'object' && 'mockReset' in value && typeof (value as any).mockReset === 'function') {
