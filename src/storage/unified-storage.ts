...
export interface StorageConfig {
  baseDir: string;              // Base directory (e.g., "sessions", "dailies", "issues")
  filePrefix: string;           // File prefix (e.g., "sessions-", "dailies-", "issues-")
  useDateSubdir: boolean;       // Whether to use date-based subdirectories
  dateExtractor?: (id: string) => string; // Extract date from ID for subdirectory
}
...
export class UnifiedStorage {
  private logger = createLogger('UnifiedStorage');
   ...
   */
  private getFilePath(config: StorageConfig, id: string): string {
   ...
   */
  async save(config: StorageConfig, item: StorageItem): Promise<void> {
   ...
   */
  async load(config: StorageConfig, id: string): Promise<StorageItem | null> {
   ...
   */
  async delete(config: StorageConfig, id: string): Promise<boolean> {
    const filePath = this.getFilePath(config, id);
   ...
   */
  async exists(config: StorageConfig, id: string): Promise<boolean> {
    const filePath = this.getFilePath(config, id);
   ...
   */
  async list(config: StorageConfig, dateDir?: string): Promise<string[]> {
    let directory = path.join(this.dataDir, config.baseDir);
   ...
   */
  async listDateDirs(config: StorageConfig): Promise<string[]> {
