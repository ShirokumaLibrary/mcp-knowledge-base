...
jest.mock('../base.js', () => ({
  BaseRepository: jest.fn().mockImplementation(function(this: any, db: any, loggerName: string) {
    this.db = db;
...
describe('SearchRepository', () => {
  ...
  describe('searchContent', () => {
    test('should search items by query in title, content, and description', async () => {
      ...
      expect(mockDb.allAsync).toHaveBeenCalledWith(
        expect.stringContaining('WHERE title LIKE ? OR content LIKE ? OR description LIKE ?'),
        ['%test%', '%test%', '%test%']
      );
    });
  ...
  describe('SQL injection protection', () => {
    test('should handle special characters in search query', async () => {
      ...
      expect(mockDb.allAsync).toHaveBeenCalledWith(
        expect.any(String),
        [`%test'; DROP TABLE items; --%`, `%test'; DROP TABLE items; --%`, `%test'; DROP TABLE items; --%`]
      );
    });
  ...
  describe('ordering', () => {
    ...
    test('searchAllByTag should order by type and created_at DESC', async () => {
      ...
      expect(mockDb.allAsync).toHaveBeenCalledWith(
        expect.stringContaining('ORDER BY type, created_at DESC'),
        expect.any(Array)
      );
    });
  });
