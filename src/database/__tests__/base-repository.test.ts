...
class TestRepository extends BaseRepository<TestEntity, number> {
  ...
  async findAll(options?: any): Promise<TestEntity[]> {
...
describe('BaseRepository', () => {
  ...
  describe('create', () => {
    test('should create new entity', async () => {
      ...
      expect(mockDb.runAsync).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO test_items'),
        expect.arrayContaining(['New Item', 'new value', 1])
      );
    });
  ...
  describe('update', () => {
    test('should update existing entity', async () => {
      ...
      expect(mockDb.runAsync).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE test_items SET'),
        expect.arrayContaining(['Updated Item', 'updated value', 1])
      );
    });
  ...
  describe('exists', () => {
    test('should return true for existing entity', async () => {
      ...
      expect(result).toBe(true);
      expect(mockDb.getAsync).toHaveBeenCalledWith(
        'SELECT 1 FROM test_items WHERE id = ? LIMIT 1',
        [1]
      );
    });
  ...
  describe('error handling', () => {
    test('should log errors with context', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Failed to find by ID in test_items',
        expect.objectContaining({ error: expect.any(Error), id: 1 })
      );
    });
  });
