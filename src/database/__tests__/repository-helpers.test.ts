...
describe('RepositoryHelpers', () => {
  ...
  describe('getNextId', () => {
    ...
    it('should handle transaction errors with rollback', async () => {
      ...
      // Execute & Assert
      await expect(
        RepositoryHelpers.getNextId(mockDb, 'issues')
    ...
    it('should log debug messages', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'Generated ID 42 for type issues'
      );
    });
  });
  ...
  describe('autoRegisterTags', () => {
    it('should create new tags', async () => {
      ...
      // Execute
      await RepositoryHelpers.autoRegisterTags(
        mockDb,
        ['tag1', 'tag2', 'tag3'],
        mockLogger
      );
  ...
  describe('saveEntityTags', () => {
    it('should save tags for entity', async () => {
      ...
      // Execute
      await RepositoryHelpers.saveEntityTags(
        mockDb,
        'issue',
        1,
        ['bug', 'urgent'],
      ...
      // Assert
      expect(mockDb.runAsync).toHaveBeenCalledWith(
        expect.stringContaining('DELETE FROM issue_tags'),
        [1]
      );
      expect(mockDb.runAsync).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO issue_tags'),
        [1, 'bug']
      );
      expect(mockDb.runAsync).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO issue_tags'),
  ...
  describe('loadEntityTags', () => {
    it('should return tags for entity', async () => {
      ...
      // Execute
      const result = await RepositoryHelpers.loadEntityTags(
        mockDb,
        'issue',
        1,
        'issue_tags',
      ...
      expect(mockDb.allAsync).toHaveBeenCalledWith(
        expect.stringContaining('SELECT tag_name FROM issue_tags'),
        [1]
      );
    });
  ...
  describe('saveRelatedEntities', () => {
    it('should save related tasks and documents', async () => {
      ...
      // Execute
      await RepositoryHelpers.saveRelatedEntities(
        mockDb,
        'issues',
        1,
        ['plans-10', 'issues-2'],
    ...
    it('should log relation saving', async () => {
      ...
      // Assert
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'Saved relationships for docs 5'
      );
    });
  });
