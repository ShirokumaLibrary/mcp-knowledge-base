...
export class ContentRepository extends BaseRepository {
   ...
   */
  async create(data: Omit<Content, 'id' | 'created_at' | 'updated_at'>): Promise<Content | null> {
    try {
  ...
  async getByType(type: string): Promise<Content[]> {
    try {
      ...
      `;
      const rows = await this.db.all<{ id: number }>(query, [type]);
   ...
   */
  async getAllSummaries(type?: string): Promise<ContentSummary[]> {
    try {
      let query = `
      ...
      const rows = await this.db.all<ContentSummary>(query, params);
      return rows.map(row => ({
        ...row,
   ...
   */
  async update(id: number, updates: Partial<Omit<Content, 'id' | 'created_at' | 'updated_at'>>): Promise<Content | null> {
    try {
   ...
   */
  async search(query: string, type?: string): Promise<ContentSummary[]> {
    try {
      let sql = `
        SELECT id, type, title, summary, tags, created_at, updated_at
        FROM search_contents
      ...
      const rows = await this.db.all<ContentSummary>(sql, params);
      return rows.map(row => ({
        ...row,
        tags: row.tags ? JSON.parse(row.tags as any) : []
      }));
   ...
   */
  private async syncContentToSQLite(content: Content): Promise<void> {
    const sql = `
  ...
  async getTypes(): Promise<string[]> {
    try {
      const rows = await this.db.all<{ type: string }>(
        'SELECT DISTINCT type FROM search_contents ORDER BY type'
      );
