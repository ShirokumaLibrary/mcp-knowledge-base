...
- data.id: string (auto-generated)
- data.related: array (unified list of all related items)
- data.related_tasks: array (for backward compatibility)
- data.related_documents: array (for backward compatibility)
```
...
Expected: Success with JSON response { data: { ... } } containing:
- data.id: string (auto-generated)
- data.related: empty array
- data.related_tasks: empty array (for backward compatibility)
- data.related_documents: empty array (for backward compatibility)
- data.tags: empty array (auto-initialized)
```
...
Expected: Success with JSON response { data: { ... } } containing:
- data.id: string (auto-generated)
- data.description: "Background worker process consumes increasing memory over time"
- data.related: empty array
```
...
Expected: Success with JSON response { data: { ... } } containing:
- data.id: string (auto-generated)
- data.related: ["issues-1", "issues-4", "docs-1", "knowledge-1"] (merged array)
- data.related_tasks: ["issues-1", "issues-4"]
- data.related_documents: ["docs-1", "knowledge-1"]
```
...
Expected: Success with JSON response { data: { id: "1", ... } }
```
...
Expected: Success with JSON response { data: { id: timestamp-based, ... } }
```
...
Expected: Success with JSON response { data: { id: date-based, ... } }
```